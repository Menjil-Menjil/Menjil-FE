...
const RegisterBasic = () => {
  const {handleNextClick, register, errors, isValid, isDirty} = useContext<any>(RegisterComponentContext);
  ...
  return (
    <>
      ...
      <FormContainerDiv>
        ...
        <InputContainer>
          ...
          <input type="text"
                 ...
                 {...register("nickname", {
                   required: true,
                   minLength: { // value의 최대 길이
                     value: 2,
                     message: "최소 2글자로 입력해주세요.", // 에러 메세지
                   },
                   maxLength: { // value의 최대 길이
                     value: 10,
        ...
        <InputContainer>
          <button type="button" className="nicknameCheckBtn" disabled={!isDirty ? true : !!errors.nickname}>중복확인</button>
        </InputContainer>
        <InputContainer>
        ...
        <InputContainer>
          ...
          <label htmlFor="choice_mentee" style={{width: "321px", height: "214px", marginRight: "25px"}}>
            <TestDiv/>
            멘티로 시작하기
          </label>
          <input type="radio"
      ...
      <GoPageBtn type="submit" disabled={!isValid} onClick={() => handleNextClick("RegisterEducation")}><RightIc/></GoPageBtn>
    </>
  );
};
